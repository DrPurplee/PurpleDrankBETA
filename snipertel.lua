...
task.spawn(function()
	...
	sound.SoundId   = "rbxassetid://1845924914"
	sound.Volume    = 0.5
	sound.Looped    = true
...
-- ══════════════════════════════════════
--  DÉTECTION JOUEURS SPÉCIFIQUES
-- ══════════════════════════════════════
local WATCH_LIST = {"IISirPuroII"}  -- Ajoute autant de pseudos que tu veux
...
-- Check au chargement (joueurs déjà présents)
task.spawn(function()
	task.wait(2)
			...
			for _, plr in pairs(Players:GetPlayers()) do
				if plr ~= LP and plr.Character then
					...
					if tRoot then
						...
						local tooFar = (d > v)
						if trackBB[plr.Name] and trackBB[plr.Name].Parent then
							trackBB[plr.Name].Enabled = not tooFar
						end
						if trackHL[plr.Name] and trackHL[plr.Name].Parent then
							trackHL[plr.Name].Enabled = not tooFar
						end
					end
				end
			end
...
-- ── TAB 8 : WATCH LIST ────────────────────────────────────────
local WatchTab = Window:CreateTab("👁️ Watch List", 4483362458)
WatchTab:CreateSection("🔔 Joueurs surveillés")
WatchTab:CreateLabel("💡 Notif auto quand ils rejoignent / quittent")

-- Affiche la liste actuelle
local function refreshWatchLabel()
	-- on recrée pas dynamiquement les labels dans Rayfield, on notifie
...
WatchTab:CreateButton({
	Name = "📋 Voir la liste",
	Callback = refreshWatchLabel,
})

WatchTab:CreateInput({
	Name = "➕ Ajouter un pseudo", PlaceholderText = "Pseudo exact...",
	...
	Callback = function(text)
		...
		notify("✅ Ajouté", text .. " surveille maintenant ✡️", 3)
	end,
})

WatchTab:CreateInput({
	Name = "➖ Retirer un pseudo", PlaceholderText = "Pseudo exact...",
	RemoveTextAfterFocusLost = false, Flag = "WatchRemove",
	Callback = function(text)
		for i, n in ipairs(WATCH_LIST) do
			if n == text then
				table.remove(WATCH_LIST, i)
				notify("🗑️ Retiré", text .. " retiré de la liste.", 3)
				return
			end
		end
...
CreditsTab:CreateLabel("🟦 Fly • Aimbot • ESP • STAFF Detect • Véhicule • Watch List")
