...
local function applySkinById(id, char)
	...
	if not ok or not desc then
		warn("[TelAviv] GetHumanoidDescription failed:", err)
		return false
	end
	local ok2, err2 = pcall(function() hum:ApplyDescription(desc) end)
	if not ok2 then
		warn("[TelAviv] ApplyDescription failed:", err2)
		return false
	end
	return true
end
...
SkinTab:CreateButton({ Name = "👤 Réappliquer maintenant",
	Callback = function()
		...
		if applySkinById(skinTargetId, char) then
			notify("👤 Skin","Skin réappliqué ✡️",2)
		else
			notify("❌","Échec — serveur bloqué ou ID invalide",3)
		end
	end })
