...
-- ══════════════════════════════════════
--  WATCH LIST (détection joueurs)
-- ══════════════════════════════════════
local WATCH_LIST = {}

local function isWatched(name)
	for _, n in ipairs(WATCH_LIST) do
...
Players.PlayerAdded:Connect(function(plr)
	if isWatched(plr.Name) then
		notify("🚨 ALERTE", plr.Name .. " vient de rejoindre ! 👀", 10)
	end
end)

Players.PlayerRemoving:Connect(function(plr)
	if isWatched(plr.Name) then
		notify("✅ PARTI", plr.Name .. " a quitté la session.", 6)
	end
end)
...
task.spawn(function()
	...
	stroke.Color        = Color3.fromRGB(30, 80, 200)
	stroke.Transparency = 0
	stroke.Parent       = frame

	local mainLabel = Instance.new("TextLabel")
	mainLabel.Size                   = UDim2.new(0, 270, 0, 40)
	mainLabel.Position               = UDim2.new(0, 16, 0, 6)
	...
	subLabel.Size                   = UDim2.new(0, 270, 0, 18)
	subLabel.Position               = UDim2.new(0, 16, 0, 47)
	subLabel.BackgroundTransparency = 1
	subLabel.Text                   = "by FocusOnTop  •  BETA"
	subLabel.TextColor3             = Color3.fromRGB(100, 150, 240)
	...
	bar.Size             = UDim2.new(0, 2, 0, 44)
	bar.Position         = UDim2.new(0, 298, 0.5, -22)
	...
	icon.Position               = UDim2.new(0, 310, 0.5, -28)
...
-- ══════════════════════════════════════════════════════════════
--  PANEL JOUEURS (ScreenGui custom)
-- ══════════════════════════════════════════════════════════════
...
local function startFollow(plr)
	stopFollow()
	followTarget = plr
	notify("👁️ Follow", "Follow ON → " .. plr.Name .. " ✡️", 3)
...
local function buildPlayerPanel()
	...
	ms.Color = Color3.fromRGB(30, 80, 200); ms.Thickness = 1.5; ms.Parent = main
	...
	sep.BackgroundColor3 = Color3.fromRGB(30, 80, 200)
	...
	searchBox.BackgroundColor3       = Color3.fromRGB(10, 20, 55)
	searchBox.BorderSizePixel        = 0
	searchBox.Text                   = ""
	searchBox.PlaceholderText        = "🔎  Rechercher un joueur..."
	searchBox.PlaceholderColor3      = Color3.fromRGB(100, 140, 220)
	...
	sbs.Color = Color3.fromRGB(30, 80, 200); sbs.Thickness = 1.2; sbs.Parent = searchBox
	...
	countLabel.TextColor3             = Color3.fromRGB(100, 150, 240)
	...
	local function makeCard(plr)
		...
		card.BackgroundColor3       = Color3.fromRGB(10, 20, 55)
		card.BackgroundTransparency = 0.2
		card.BorderSizePixel        = 0
		card.Parent                 = scroll
		Instance.new("UICorner", card).CornerRadius = UDim.new(0, 8)
		local cs = Instance.new("UIStroke")
		cs.Color = Color3.fromRGB(25, 70, 180); cs.Thickness = 1; cs.Parent = card

		-- Avatar
		local avatar = Instance.new("ImageLabel")
		avatar.Size                   = UDim2.new(0, 42, 0, 42)
		avatar.Position               = UDim2.new(0, 8, 0.5, -21)
		avatar.BackgroundColor3       = Color3.fromRGB(15, 30, 80)
		...
		statusLabel.Size                   = UDim2.new(1, -110, 0, 18)
		statusLabel.Position               = UDim2.new(0, 58, 0, 32)
		statusLabel.BackgroundTransparency = 1
		statusLabel.Text                   = getPlayerStatus(plr)
		statusLabel.TextColor3             = Color3.fromRGB(100, 150, 230)
		statusLabel.Font                   = Enum.Font.Gotham
		statusLabel.TextSize               = 12
		statusLabel.TextXAlignment         = Enum.TextXAlignment.Left
		statusLabel.Parent                 = card

		-- Update status en live
		task.spawn(function()
		...
		followBtn.Size                   = UDim2.new(0, 36, 0, 36)
		followBtn.Position               = UDim2.new(1, -88, 0.5, -18)
		followBtn.BackgroundColor3       = Color3.fromRGB(20, 60, 180)
		followBtn.Text                   = "👁️"
		...
		followBtn.MouseButton1Click:Connect(function()
			if followTarget == plr then
				stopFollow()
				followBtn.BackgroundColor3 = Color3.fromRGB(20, 60, 180)
				notify("👁️ Follow", "Follow OFF ✡️", 2)
			else
				-- Reset couleur ancien bouton
				for _, c in pairs(playerCards) do
					if c.btn and c.btn.Parent then
						c.btn.BackgroundColor3 = Color3.fromRGB(20, 60, 180)
					end
				end
				startFollow(plr)
				followBtn.BackgroundColor3 = Color3.fromRGB(50, 200, 100)
			end
		end)

		-- Bouton 📡 TP
		local tpBtn = Instance.new("TextButton")
		tpBtn.Size                   = UDim2.new(0, 36, 0, 36)
		tpBtn.Position               = UDim2.new(1, -46, 0.5, -18)
		tpBtn.BackgroundColor3       = Color3.fromRGB(20, 60, 180)
		tpBtn.Text                   = "📡"
		tpBtn.TextSize               = 18
		...
		return { card = card, name = plr.Name, btn = followBtn }
	end
	...
	local function buildList(filter)
		...
		countLabel.Text = count .. " joueur(s) affiché(s)  •  " .. #Players:GetPlayers() .. " en session"
	end
	...
	-- Refresh auto quand quelqu'un rejoint/quitte
	Players.PlayerAdded:Connect(function()
		task.wait(1); buildList(searchBox.Text)
	end)
	Players.PlayerRemoving:Connect(function()
		task.wait(0.2); buildList(searchBox.Text)
	end)
end
...
local function sFLY()
	...
	flyKeyDown = UIS.InputBegan:Connect(function(input, gpe)
		...
		elseif input.KeyCode == Enum.KeyCode.E then CTRL.U = 1
		elseif input.KeyCode == Enum.KeyCode.Q then CTRL.D = 1
		end
	end)
...
PlayerTab:CreateSlider({
	Name = "✈️ Fly Speed", Range = {10,500}, Increment = 5,
	Suffix = " st/s", CurrentValue = 80, Flag = "FlySpeed",
	Callback = function(v)
		flySpeed = v
		notify("✈️ Fly Speed", v.." st/s ✡️", 1, 0.8)
	end,
})
...
PlayerTab:CreateLabel("💡 Speed réglable pendant le vol en temps réel ✡️")
...
JoueursTab:CreateLabel("💡 Ouvrir le panel : cherche, follow, TP en 1 clic")
JoueursTab:CreateButton({
	Name = "👥 Ouvrir / Fermer le Panel",
	Callback = function()
		buildPlayerPanel()
	end,
})
JoueursTab:CreateSection("🔔 Watch List — Alertes")
JoueursTab:CreateLabel("💡 Notif auto quand un pseudo surveillé rejoint / quitte")
JoueursTab:CreateButton({
	Name = "📋 Voir la liste",
	Callback = function()
...
JoueursTab:CreateInput({
	Name = "➕ Ajouter à la Watch List", PlaceholderText = "Pseudo exact...",
	RemoveTextAfterFocusLost = false, Flag = "WatchAdd",
	Callback = function(text)
		if text == "" then return end
		for _, n in ipairs(WATCH_LIST) do
			if n == text then notify("⚠️","Déjà dans la liste !",2); return end
		end
		table.insert(WATCH_LIST, text)
		notify("✅ Ajouté", text .. " surveillé ✡️", 3)
	end,
})
JoueursTab:CreateInput({
	Name = "➖ Retirer de la Watch List", PlaceholderText = "Pseudo exact...",
	RemoveTextAfterFocusLost = false, Flag = "WatchRemove",
	Callback = function(text)
		for i, n in ipairs(WATCH_LIST) do
			if n == text then
				table.remove(WATCH_LIST, i)
				notify("🗑️ Retiré", text .. " retiré.", 3)
				return
			end
		end
...
JoueursTab:CreateButton({
	Name = "🔍 Scan — Sont-ils présents ?",
	Callback = function()
		local found = {}
		for _, name in ipairs(WATCH_LIST) do
			for _, plr in pairs(Players:GetPlayers()) do
				if plr.Name == name then table.insert(found, name) end
			end
		end
		if #found == 0 then notify("🔍 Scan","Aucun joueur surveillé présent.",4)
		else notify("🚨 PRÉSENTS !", table.concat(found,"  |  "), 8) end
	end,
})
...
ESPTab:CreateSlider({
	Name = "📏 Distance d'affichage ESP", Range = {10, 5000}, Increment = 10,
	Suffix = " st", CurrentValue = 1000, Flag = "ESPMaxDist",
	Callback = function(v)
		espMaxDistance = v
		local myRoot = getHRP()
		if myRoot then
			for _, plr in pairs(Players:GetPlayers()) do
				if plr ~= LP and plr.Character then
					...
					if tRoot then
						...
						local tooFar = (d > v)
						if trackBB[plr.Name] and trackBB[plr.Name].Parent then trackBB[plr.Name].Enabled = not tooFar end
						if trackHL[plr.Name] and trackHL[plr.Name].Parent then trackHL[plr.Name].Enabled = not tooFar end
					end
				end
			end
		end
...
MiscTab:CreateToggle({
	Name = "🎵 Musique ON/OFF", CurrentValue = true, Flag = "MusicToggle",
	Callback = function(v)
		if v then
			bgMusic:Play()
			notify("🎵 Musique", "ON ✡️ — Let it play 🎶", 2)
		else
			bgMusic:Stop()
			notify("🎵 Musique", "OFF 🛑 — Silence", 2)
		end
	end,
})
MiscTab:CreateSlider({
	Name = "🔊 Volume", Range = {0, 100}, Increment = 5,
	Suffix = "%", CurrentValue = 50, Flag = "MusicVolume",
	Callback = function(v)
		bgMusic.Volume = v / 100
		notify("🔊 Volume", v.."%  ✡️", 1, 0.6)
	end,
})
MiscTab:CreateButton({
	Name = "⏭️ Changer le son (ID custom)",
	Callback = function()
		notify("⏭️ Son", "Modifie bgMusic.SoundId dans le script ✡️", 4)
	end,
})
...
CreditsTab:CreateLabel("🟦 Fly • Aimbot • ESP • STAFF Detect • Véhicule • Panel Joueurs")
