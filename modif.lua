...
local Window = Rayfield:CreateWindow({
	Name            = "✡️ Tel-Aviv Menu ~ By Cyril Hanouna",
	LoadingTitle    = "Tel-Aviv [Menu]",
...
local function applySkinById(id, char)
    ...
    -- Méthode 1 : ApplyDescription forcé
    local desc = nil
    ...
    -- Méthode 2 : GetCharacterAppearanceAsync en complément
...
task.spawn(function()
	...
	mainLabel.BackgroundTransparency = 1; mainLabel.Text = "TEL-AVIV Menu"
...
PlayerTab:CreateLabel("💡 CTRL Gauche = ON/OFF")
...
JoueursTab:CreateLabel("👁️ = Surveiller   📡 = TP sur le joueur")
JoueursTab:CreateButton({ Name = "👥 Ouvrir  le Panel", Callback = function() buildPlayerPanel() end })
...
CombatTab:CreateToggle({ Name = "👊 Push ALL", CurrentValue = false, Flag = "PushAura",
	Callback = function(v)
		pushAuraActive = v
		if v then
...
VehTab:CreateLabel("💡 Active → clic gauche ")
...
VehTab:CreateSlider({ Name = "🚀 Vehicle Fly Speed", Range = {10,500}, Increment = 10, Suffix = " st/s", CurrentValue = 100, Flag = "VehFlySpeed",
	Callback = function(v)
		vehicleSpeed = v
...
CreditsTab:CreateLabel("👑 FocusOnTop"); CreditsTab:CreateLabel("💬 discord.gg/focuscheater")
CreditsTab:CreateSection("Build")
CreditsTab:CreateLabel("✡️ Tel-Aviv — Menu")
...
notify("✡️ Tel-Aviv","Menu chargé ! — By FocusOnTop",5)
