...
-- ══════════════════════════════════════════════════════════════
--  SKIN LOCAL — ID DIRECT (plus fiable que GetUserIdFromNameAsync)
-- ══════════════════════════════════════════════════════════════
local skinTargetId    = 8069446587   -- ← ID Roblox direct, pas de fetch
local skinTargetName  = "cutieinaria"
local skinAutoReapply = true

local function applyDescManual(desc, char)
	if not desc or not char then return false end
	...
	pcall(function()
		local parts = {
			Head=desc.HeadColor, UpperTorso=desc.TorsoColor, LowerTorso=desc.TorsoColor,
	...
	pcall(function()
		...
		if desc.Shirt ~= 0 then local s = Instance.new("Shirt"); s.ShirtTemplate = "rbxassetid://"..desc.Shirt; s.Parent = char end
		if desc.Pants ~= 0 then local p = Instance.new("Pants"); p.PantsTemplate = "rbxassetid://"..desc.Pants; p.Parent = char end
	end)
...
local function applySkinById(id, char)
	...
	if not ok or not desc then warn("[Skin] failed:", err); return false end
	return applyDescManual(desc, char)
end

-- Application au lancement
task.spawn(function()
	local char = LP.Character or LP.CharacterAdded:Wait()
	task.wait(1.5)  -- attend que le perso soit bien chargé
	if applySkinById(skinTargetId, char) then
		notify("👤 Skin", "Skin de "..skinTargetName.." appliqué ✡️", 3)
	else
		notify("👤 Skin", "Échec — réessaie dans 2s...", 3)
		-- Retry automatique
		task.wait(2)
		local char2 = LP.Character
		if char2 then
			if applySkinById(skinTargetId, char2) then
				notify("👤 Skin", "Skin appliqué (retry) ✡️", 3)
			else
				notify("👤 Skin", "Échec définitif — serveur bloqué ?", 4)
			end
		end
	end
end)
...
JoueursTab:CreateLabel("💡 👁️ = SPEC caméra   📡 = TP sur le joueur")
...
SkinTab:CreateLabel("💡 Skin appliqué via ID direct — local seulement")
SkinTab:CreateLabel("💡 Les autres joueurs voient ton vrai skin")
...
SkinTab:CreateButton({ Name = "🔄 Reset — Mon vrai skin",
	Callback = function()
		...
		if desc then
			skinTargetId   = LP.UserId
			skinTargetName = LP.Name
			applyDescManual(desc, char)
			notify("🔄 Reset","Ton vrai skin restauré ✡️",2)
		end
	end })
SkinTab:CreateInput({ Name = "🔁 Changer — ID Roblox", PlaceholderText = "Ex: 8069446587", RemoveTextAfterFocusLost = false, Flag = "SkinTarget",
	Callback = function(text)
		if text == "" then return end
		local newId = tonumber(text)
		if not newId then
			-- Si c'est un pseudo on essaie quand même
			task.spawn(function()
				...
				if ok and id then
					...
					if char and applySkinById(skinTargetId, char) then
						...
						notify("❌","Échec d'application",3)
					end
				else
					notify("❌", "'"..text.."' introuvable — entre un ID numérique", 3)
				end
			end)
			return
		end
		skinTargetId   = newId
		skinTargetName = "ID:"..newId
		local char = LP.Character
		if char and applySkinById(skinTargetId, char) then
			notify("👤 Skin", "Skin (ID "..newId..") appliqué ✡️", 3)
		else
			notify("❌","Échec — ID invalide ?",3)
		end
	end })
SkinTab:CreateButton({ Name = "🔄 Reset — Mon vrai skin",
	Callback = function()
...
ESPTab:CreateSlider({ Name = "📏 Distance d'affichage ESP", Range = {10,5000}, Increment = 10, Suffix = " st", CurrentValue = 1000, Flag = "ESPMaxDist",
	Callback = function(v) espMaxDistance = v; notify("📏 ESP Distance", "Affiche jusqu'à "..v.." st ✡️", 1, 0.8) end })
