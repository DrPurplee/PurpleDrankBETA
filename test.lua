...
-- ══════════════════════════════════════
--  DÉTECTION JOUEUR SPÉCIFIQUE
-- ══════════════════════════════════════
local TARGET_NAME = "IISirPuroII"

local function checkTargetPresent()
	for _, plr in pairs(Players:GetPlayers()) do
		if plr.Name == TARGET_NAME then
			notify("⚠️ ALERTE", TARGET_NAME .. " est dans la session ! 👀", 8)
			return
		end
	end
end

-- Check au chargement du script
task.wait(2)
checkTargetPresent()

-- Check à chaque nouveau joueur qui rejoint
Players.PlayerAdded:Connect(function(plr)
	if plr.Name == TARGET_NAME then
		notify("⚠️ ALERTE", TARGET_NAME .. " vient de rejoindre ! 👀", 8)
	end
end)

-- Notif quand il quitte aussi
Players.PlayerRemoving:Connect(function(plr)
	if plr.Name == TARGET_NAME then
		notify("✅ PARTI", TARGET_NAME .. " a quitté la session.", 5)
	end
end)
...
local TARGETS = {"IISirPuroII", "AutreJoueur", "Encore1"}

Players.PlayerAdded:Connect(function(plr)
	for _, name in ipairs(TARGETS) do
		if plr.Name == name then
			notify("⚠️ ALERTE", name .. " vient de rejoindre ! 👀", 8)
		end
	end
end)
...
-- ══════════════════════════════════════════════════════════════
--  FLY — vitesse dynamique corrigée
-- ══════════════════════════════════════════════════════════════
local FLYING     = false
local flySpeed   = 80   -- modifié dynamiquement par le slider
	...
	task.spawn(function()
		repeat
			...
			if hum then
				...
				-- maintien HP max pendant le vol
				if hum.Health < hum.MaxHealth then hum.Health = hum.MaxHealth end
			end

			local dir = Vector3.zero
			-- FIX : on utilise flySpeed directement (lu à chaque frame)
			...
			if dir.Magnitude > 0 then
				BV.Velocity = dir.Unit * flySpeed   -- ← flySpeed lu en temps réel
			else
				BV.Velocity = Vector3.zero
			end
...
local espStudsOffset = 5   -- ← valeur par défaut, modifiable par slider
...
local function setupTrackChar(plr, char)
	...
	bb.StudsOffset  = Vector3.new(0, espStudsOffset, 0)  -- ← dynamique
	bb.AlwaysOnTop  = true
	bb.Parent       = hrp
	trackBB[plr.Name] = bb
	...
	local dc = RunService.RenderStepped:Connect(function()
		...
		-- Mise à jour dynamique du StudsOffset selon le slider
		if bb and bb.Parent then
			bb.StudsOffset = Vector3.new(0, espStudsOffset, 0)
		end
...
-- FIX : ce slider modifie flySpeed directement, appliqué en temps réel même pendant le vol
PlayerTab:CreateSlider({
	Name = "✈️ Fly Speed", Range = {10,500}, Increment = 5,
	Suffix = " st/s", CurrentValue = 80, Flag = "FlySpeed",
	Callback = function(v)
		flySpeed = v
		notify("✈️ Fly Speed", v.." st/s ✡️", 1)
	end,
})
...
-- NOUVEAU : slider pour régler la hauteur d'affichage des tags ESP
ESPTab:CreateSlider({
	Name = "📐 Hauteur affichage ESP", Range = {-10, 30}, Increment = 1,
	Suffix = " st", CurrentValue = 5, Flag = "ESPStuds",
	Callback = function(v)
		espStudsOffset = v
		-- Applique immédiatement à tous les BillboardGui existants
		for _, plr in pairs(Players:GetPlayers()) do
			if trackBB[plr.Name] and trackBB[plr.Name].Parent then
				trackBB[plr.Name].StudsOffset = Vector3.new(0, v, 0)
			end
		end
		notify("📐 ESP Hauteur", v.." studs ✡️", 1)
	end,
})
