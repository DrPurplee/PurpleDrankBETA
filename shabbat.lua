...
-- DISTANCE MAX D'AFFICHAGE ESP (en studs) — modifiable par slider
local espMaxDistance = 1000  -- par défaut : 1000 studs (tout le monde visible)
...
local function setupTrackChar(plr, char)
	...
	local dc = RunService.RenderStepped:Connect(function()
		...
		if myRoot then
			...
			-- Cache tout l'ESP (highlight + tag) si le joueur est trop loin
...
-- Fly Speed : cooldown de 0.8s sur la notif pour éviter le spam slider
PlayerTab:CreateSlider({
	Name = "✈️ Fly Speed", Range = {10,500}, Increment = 5,
	Suffix = " st/s", CurrentValue = 80, Flag = "FlySpeed",
	Callback = function(v)
		flySpeed = v
		notify("✈️ Fly Speed", v.." st/s ✡️", 1, 0.8)  -- cooldown 0.8s anti-spam
	end,
})
...
-- ═══════════════════════════════════════════════════════════════
--  DISTANCE D'AFFICHAGE ESP — filtre les joueurs trop loin
--  Range : 10 → 5000 studs (5000 = tout le monde visible)
-- ═══════════════════════════════════════════════════════════════
ESPTab:CreateSlider({
	...
	Callback = function(v)
		espMaxDistance = v
		-- Applique immédiatement : cache/montre les BillboardGui et Highlight existants
		local myRoot = getHRP()
		if myRoot then
			for _, plr in pairs(Players:GetPlayers()) do
				if plr ~= LP and plr.Character then
		...
		-- Notif avec cooldown anti-spam 0.8s
